...
STATUS iter_CreateIterator(Iterator** iterator)
{
    ...
    *iterator = iteratorList + emptyIteratorIndex;
    ...
    dbg_LogInformation("All iterators used up");
...
STATUS iter_AddNode(Iterator* iterator, void* data)
{
    ...
    dbg_LogInformation("Adding node[%i] to iterator[%i]", emptyNodeIndex, id);
...
/* Get prev node. If startNode has no prev, prevNode is set to NULL and returns
 * ERROR*/
STATUS iter_GetPrev(Node* startNode, Node** prevNode)
{
    int16 id = startNode - nodeList;
